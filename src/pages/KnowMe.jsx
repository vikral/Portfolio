...
/* ── Philosophy Card (Extracted to fix Hook violation) ── */
function PhilosophyCard({ item, delay }) {
...
/* ── Movie Card (Extracted to fix Hook violation) ── */
function MovieCard({ film, delay }) {
  const [hover, setHover] = useState(false)
  const { isMobile } = useResponsive()
  return (
    <FadeIn delay={delay}>
      <div onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
        ...
        <p style={{fontSize:'14px',color:hover?'#888':'#444',fontWeight:300,lineH:1.7,transition:'color .3s'}}>{film.note}</p>
      </div>
    </FadeIn>
  )
}
...
function TwoTruthsLie() {
  ...
  const options = [
    { text: "I once spent 48 hours straight debugging a WordPress plugin for a client in the US.", isLie: false },
    { text: "I can ride a bike from Noida to the hills in a single stretch without stopping for more than an hour.", isLie: false },
    { text: "I prefer coffee over chai for my midnight coding sessions.", isLie: true },
  ]
  ...
  return (
    <div style={{
      ...
      <div style={{fontFamily:"'Syne',sans-serif",fontSize:'11px',fontWeight:700,letterSpacing:'.3em',textTransform:'uppercase',color:'var(--accent)',display:'flex',alignItems:'center',justifyContent:'center',gap:'12px',marginBottom:'20px'}}>
        <span style={{width:'24px',height:'1px',background:'var(--accent)',display:'block'}}/>
        Interactive Challenge
      </div>
      <h2 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'clamp(44px,6vw,80px)',lineHeight:.9,letterSpacing:'-.01em',color:'var(--ink)',marginBottom:'12px'}}>
        Two Truths <em style={{fontFamily:"'DM Serif Display',serif",fontStyle:'italic',color:'var(--accent)'}}>& One Lie</em>
      </h2>
      <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:'14px',color:'var(--mid)',marginBottom:'40px',maxWidth:'500px',marginInline:'auto'}}>
        Three statements. Two are absolute facts. One is a complete fabrication. Can you spot the lie?
      </p>
        ...
        {options.map((opt, i) => (
          <div
            ...
            style={{
              padding: '32px 24px',
              background: revealed
                ? (opt.isLie ? 'var(--accent)' : 'var(--ink)')
                : 'var(--ink)',
              color: revealed
                ? (opt.isLie ? '#fff' : 'var(--cream)')
                : 'var(--cream)',
              border: '1px solid var(--light)',
              borderRadius: '2px',
              ...
              lineHeight: 1.6,
              position: 'relative',
              boxShadow: selected === i ? '0 0 0 3px var(--accent)' : 'none'
            }}
          >
            {opt.text}
            {revealed && opt.isLie && (
              <span style={{position:'absolute',top:'10px',right:'10px',fontSize:'12px',fontWeight:700,color:'#fff'}}>LIE!</span>
            )}
          </div>
        ))}
      </div>
      ...
      {revealed && (
        <FadeIn delay={200}>
          <div style={{marginTop:'40px', display:'flex', flexDirection:'column', alignItems:'center', gap:'16px'}}>
            <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:'16px',color:'var(--ink)',fontWeight:500}}>
              {selected === 2
                ? "Correct! You caught the lie. I'm a die-hard chai lover."
                : "Nope! The lie was the coffee one. Chai is non-negotiable."}
            </p>
            <button
          ...
          <div style={{position:'relative',zIndex:1,display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:isMobile?'48px':'80px',alignItems:'start'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'28px'}}>
              {[
                ...
                { marker:'02', text:"College hostel changed everything. My senior — final year CS — showed me my first <div style={{display:'inline'}}>tag</div> at 1AM on his laptop. Something clicked. Not the code. The power. You could build a world from nothing but text." },
                ...
                { marker:'04', text:"5+ years. 3 companies. 100+ projects. That Bihari kid staring at websites didn't know he was looking at his future." },
              ].map((item,i) => (
                <FadeIn key={i} delay={i*80}>
                  <div style={{display:'flex',gap:'20px',alignItems:'flex-start'}}>
                    ...
                    <p style={{fontSize:'15px',color:'#666',fontWeight:300,lineH:1.9}}>{item.text}</p>
                  </div>
                </FadeIn>
              ))}
        ...
        <section style={{background:'var(--cream)',padding:`${pyLg} ${px}`,borderTop:'1px solid var(--light)'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':isTablet?'repeat(2,1fr)':'repeat(3,1fr)',gap:'1px',background:'var(--light)',border:'1px solid var(--light)'}}>
            {philosophy.map((item,i) => (
              <PhilosophyCard key={i} item={item} delay={i*60} />
            ))}
          </div>
        </section>
